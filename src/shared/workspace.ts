// Which files a code run wrote Kiln will show or hand to other apps. A file a run wrote may carry the chat's data, and
// whatever opens it runs outside the sandbox: an app that runs a document's scripts, or loads its remote images and
// templates, could send that data out (#67). So on a Mac, Kiln previews files with Quick Look instead of opening them
...
/** How Kiln opens a file a run wrote on this platform, or null when it won't. */
