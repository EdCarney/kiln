...
- **Code runner.** Models that support tools can run Python 3 or bash with `run_code`, switched on per chat from the + menu under **Tools** (Settings → Tools → Code runner sets whether it asks first, whether new chats start with it, PyPI access and the time limit). It also answers gpt-oss's built-in `python` tool.
  ...
  - Kiln works in a chat's folder outside the sandbox (copying uploads in, listing what a run wrote, marking files), so it never follows a link code left there: it does that only while none of the chat's code is running (Show in Finder waits for a run to finish), replacing a link with a folder where it expects one; it opens a file to preview, save or show only if no part of its path is a link; and a preview is of a copy, never the file in the chat's folder. The script a run executes is kept outside the folder (`runner/scripts/<chat id>`), where code can read it but not change it.
